...
interface ExperienceData {
  title?: string;
  subtitle?: string;
  entries?: ExperienceItemData[];
}

export function ExperienceComponent({ section }: { section: SectionType<ExperienceData> }) {
  const entries = section.data?.entries || [];

  return (
    <LayoutSection
      id={section.id || "experience"}
    ...
    >
      <div className="flex flex-col">
        ...
        <p className="mt-6 max-w-[60ch] text-[15px] leading-[1.6] text-muted-foreground">
          {section.data?.subtitle || "Roles held across engineering and platform teams, reverse-chronological."}
        </p>
      </div>
